...
const InlineCarousel = ({ title, images }) => {
  ...
  return (
    <div style={{ margin: '1rem 0', padding: '0.75rem', background: 'white', borderRadius: '16px', border: '1px solid #e2e8f0', boxShadow: '0 4px 12px rgba(0,0,0,0.05)' }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', marginBottom: '0.75rem' }}>
        <div style={{ width: '28px', height: '28px', borderRadius: '50%', background: '#f0fdf4', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#16a34a' }}>
          <ImageIcon size={14} />
        </div>
        <h4 style={{ margin: 0, color: '#0f172a', fontSize: '0.9rem', fontWeight: 'bold' }}>{title} Gallery</h4>
      </div>
      
      <div style={{ position: 'relative', width: '100%', height: '220px', borderRadius: '10px', overflow: 'hidden' }}>
        {images.map((img, idx) => (
          <img 
            key={idx}
        ...
        {images.length > 1 && (
          <>
            <button 
              onClick={prevSlide}
              style={{ position: 'absolute', left: '10px', top: '50%', transform: 'translateY(-50%)', background: 'rgba(0,0,0,0.5)', color: 'white', border: 'none', borderRadius: '50%', width: '32px', height: '32px', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', zIndex: 10, boxShadow: '0 2px 4px rgba(0,0,0,0.2)' }}
            ><ChevronLeft size={20}/></button>
            <button 
              onClick={nextSlide}
              style={{ position: 'absolute', right: '10px', top: '50%', transform: 'translateY(-50%)', background: 'rgba(0,0,0,0.5)', color: 'white', border: 'none', borderRadius: '50%', width: '32px', height: '32px', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', zIndex: 10, boxShadow: '0 2px 4px rgba(0,0,0,0.2)' }}
            ><ChevronRight size={20}/></button>
            
            <div style={{ position: 'absolute', bottom: '10px', left: '50%', transform: 'translateX(-50%)', display: 'flex', gap: '6px', zIndex: 10 }}>
              {images.map((_, idx) => (
                <div key={idx} style={{ width: '8px', height: '8px', borderRadius: '50%', background: idx === currentIndex ? 'white' : 'rgba(255,255,255,0.5)', cursor: 'pointer', boxShadow: '0 1px 2px rgba(0,0,0,0.3)' }} onClick={() => setCurrentIndex(idx)} />
              ))}
            </div>
          </>
        )}
...
const InlineMap = ({ markers, onNavigate }) => {
  ...
  return (
    <div style={{ padding: '0.75rem', background: 'white', borderRadius: '16px', border: '1px solid #e2e8f0', boxShadow: '0 4px 12px rgba(0,0,0,0.05)', height: '100%', minHeight: '300px', display: 'flex', flexDirection: 'column' }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', marginBottom: '0.75rem' }}>
        <div style={{ width: '28px', height: '28px', borderRadius: '50%', background: '#fef2f2', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#ef4444' }}>
          <MapPin size={14} />
        </div>
        <h4 style={{ margin: 0, color: '#0f172a', fontSize: '0.9rem', fontWeight: 'bold' }}>Map Overview</h4>
      </div>
              ...
              <Popup>
                <div style={{ textAlign: 'center', minWidth: '140px' }}>
                  {m.image && m.image.trim() !== '' && (
                    <img src={m.image.trim().startsWith('http') ? m.image.trim() : `http://localhost:8000${m.image.trim()}`} alt={m.title} style={{ width: '100%', height: '80px', objectFit: 'cover', borderRadius: '8px', marginBottom: '8px' }} />
                  )}
                  <h4 style={{ margin: '0 0 8px 0', fontSize: '0.9rem', color: '#0f172a' }}>{m.title}</h4>
                  {m.link && m.link.trim() !== '' && (
                    <a 
                      href={m.link.trim()}
                      onClick={(e) => {
                      ...
                      style={{ display: 'inline-block', padding: '6px 12px', background: 'var(--color-1)', color: 'white', borderRadius: '20px', textDecoration: 'none', fontSize: '0.8rem', fontWeight: 'bold' }}
                    >
                      View Details
                    </a>
                  )}
...
const InlineBookingForm = ({ roomId, initialInDate, initialOutDate, onSubmit }) => {
  ...
  return (
    <div style={{ marginTop: '1rem', padding: '1.25rem', background: 'white', borderRadius: '16px', border: '1px solid #e2e8f0', boxShadow: '0 4px 12px rgba(0,0,0,0.05)' }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', marginBottom: '1rem' }}>
        ...
        <h4 style={{ margin: 0, color: '#0f172a', fontSize: '1rem' }}>Booking Confirmation</h4>
      </div>
      ...
      <div style={{ display: 'flex', gap: '0.75rem', marginBottom: '1rem' }}>
        <div style={{ flex: 1 }}>
          <label style={{ display: 'block', fontSize: '0.75rem', fontWeight: 'bold', color: '#475569', marginBottom: '0.4rem', textTransform: 'uppercase', letterSpacing: '0.05em' }}>Check In</label>
          <input type="date" value={inDate} onChange={e => setInDate(e.target.value)} style={{ width: '100%', padding: '0.65rem', borderRadius: '10px', border: '1px solid #cbd5e1', outline: 'none', transition: 'border-color 0.2s', fontFamily: 'inherit' }} onFocus={(e) => e.target.style.borderColor = 'var(--color-1)'} onBlur={(e) => e.target.style.borderColor = '#cbd5e1'} />
        </div>
        <div style={{ flex: 1 }}>
          <label style={{ display: 'block', fontSize: '0.75rem', fontWeight: 'bold', color: '#475569', marginBottom: '0.4rem', textTransform: 'uppercase', letterSpacing: '0.05em' }}>Check Out</label>
          <input type="date" value={outDate} onChange={e => setOutDate(e.target.value)} style={{ width: '100%', padding: '0.65rem', borderRadius: '10px', border: '1px solid #cbd5e1', outline: 'none', transition: 'border-color 0.2s', fontFamily: 'inherit' }} onFocus={(e) => e.target.style.borderColor = 'var(--color-1)'} onBlur={(e) => e.target.style.borderColor = '#cbd5e1'} />
        </div>
      </div>
      
      <div style={{ marginBottom: '1.25rem' }}>
        <label style={{ display: 'block', fontSize: '0.75rem', fontWeight: 'bold', color: '#475569', marginBottom: '0.4rem', textTransform: 'uppercase', letterSpacing: '0.05em' }}>Payment Proof</label>
        <div style={{ position: 'relative', overflow: 'hidden' }}>
          <input type="file" accept="image/*" onChange={e => setFile(e.target.files[0])} style={{ width: '100%', padding: '0.5rem', background: '#f8fafc', borderRadius: '10px', border: '1px dashed #cbd5e1', fontSize: '0.85rem', color: '#64748b', cursor: 'pointer' }} />
        </div>
      </div>

      <button onClick={handleSubmit} style={{ width: '100%', padding: '0.85rem', background: 'linear-gradient(135deg, var(--color-1) 0%, var(--color-3) 100%)', color: 'white', border: 'none', borderRadius: '12px', fontWeight: 'bold', fontSize: '0.95rem', cursor: 'pointer', boxShadow: '0 4px 12px rgba(0,0,0,0.1)', transition: 'transform 0.2s' }} onMouseOver={(e) => e.currentTarget.style.transform = 'translateY(-2px)'} onMouseOut={(e) => e.currentTarget.style.transform = 'none'}>
        Confirm & Book Room
      </button>
    </div>
  );
...
const InlineBookingWidget = ({ roomId, inDate, outDate, price, onConfirm }) => {
  return (
    <div style={{ background: 'linear-gradient(135deg, #0f172a 0%, #1e293b 100%)', borderRadius: '16px', padding: '1.5rem', color: 'white', border: '1px solid #334155', boxShadow: '0 10px 25px rgba(0,0,0,0.2)', position: 'relative', overflow: 'hidden' }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem', marginBottom: '1rem', position: 'relative', zIndex: 2 }}>
        ...
        <h3 style={{ margin: 0, fontSize: '1.1rem', color: '#10b981' }}>1-Click Reservation</h3>
      </div>
      <div style={{ background: 'rgba(255,255,255,0.05)', borderRadius: '12px', padding: '1rem', marginBottom: '1.25rem', position: 'relative', zIndex: 2 }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '0.5rem' }}>
          <span style={{ color: '#94a3b8', fontSize: '0.85rem' }}>Check In</span>
          <span style={{ fontWeight: 'bold' }}>{inDate}</span>
        </div>
        <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '0.5rem' }}>
          <span style={{ color: '#94a3b8', fontSize: '0.85rem' }}>Check Out</span>
          <span style={{ fontWeight: 'bold' }}>{outDate}</span>
        </div>
        <div style={{ borderTop: '1px solid rgba(255,255,255,0.1)', margin: '0.75rem 0' }} />
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <span style={{ color: '#94a3b8', fontSize: '0.9rem' }}>Total Price</span>
          <span style={{ fontWeight: 'bold', color: 'var(--color-1)', fontSize: '1.25rem' }}>EGP {price}</span>
        </div>
      </div>
      <button 
        onClick={() => onConfirm(roomId, inDate, outDate)}
        style={{ width: '100%', padding: '0.85rem', background: 'var(--color-1)', color: 'white', border: 'none', borderRadius: '10px', fontWeight: 'bold', fontSize: '1rem', cursor: 'pointer', boxShadow: '0 4px 12px rgba(16, 185, 129, 0.3)', transition: 'transform 0.2s', position: 'relative', zIndex: 2 }}
...
const InlineItinerary = ({ title, daysData }) => {
  return (
    <div style={{ background: 'white', borderRadius: '16px', padding: '1.5rem', border: '1px solid #e2e8f0', boxShadow: '0 4px 12px rgba(0,0,0,0.05)', marginTop: '1rem', marginBottom: '1rem' }}>
       <h3 style={{ margin: '0 0 1.25rem 0', color: 'var(--color-1)', borderBottom: '2px solid #f1f5f9', paddingBottom: '0.75rem', display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
         🗺️ Trip to {title}
       </h3>
       <div style={{ display: 'flex', flexDirection: 'column', gap: '0' }}>
         {daysData.map((dayObj, dIdx) => (
           <div key={dIdx} style={{ display: 'flex', gap: '1.25rem' }}>
             {/* Timeline Visual Line */}
             <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
               <div style={{ width: '32px', height: '32px', borderRadius: '50%', backgroundColor: 'var(--color-1)', color: 'white', display: 'flex', alignItems: 'center', justifyContent: 'center', fontWeight: 'bold', zIndex: 2, boxShadow: '0 2px 6px rgba(16, 185, 129, 0.3)' }}>
                 {dIdx + 1}
               </div>
               {dIdx < daysData.length - 1 && (
                 <div style={{ width: '2px', flex: 1, backgroundColor: '#e2e8f0', margin: '0.25rem 0' }} />
               )}
             </div>
             
             {/* Timeline Content */}
             <div style={{ flex: 1, paddingBottom: dIdx < daysData.length - 1 ? '2rem' : '0' }}>
               <h4 style={{ margin: '0 0 0.75rem 0', color: '#334155', fontSize: '1.05rem', paddingTop: '0.25rem' }}>{dayObj.label}</h4>
               <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
                 {dayObj.activities.map((act, aIdx) => (
                   <div 
                     key={aIdx} 
                     style={{ padding: '0.85rem 1rem', backgroundColor: '#f8fafc', borderRadius: '10px', border: '1px solid #e2e8f0', display: 'flex', alignItems: 'flex-start', gap: '0.75rem', cursor: 'grab', transition: 'all 0.2s' }} 
                     draggable="true"
                     onMouseOver={(e) => { e.currentTarget.style.borderColor = 'var(--color-1)'; e.currentTarget.style.backgroundColor = 'white'; }}
                   ...
                   >
                     <span style={{ fontSize: '1rem', color: 'var(--color-1)', marginTop: '-0.1rem' }}>✦</span>
                     <span style={{ fontSize: '0.95rem', color: '#475569', lineHeight: '1.5' }}>{act}</span>
                   </div>
                 ))}
               </div>
             </div>
...
const Chatbot = () => {
  ...
  const [messages, setMessages] = useState([
    {
      sender: 'bot',
      text: "Marhaban! 🌴 Welcome to OasisStays. I'm your AI concierge. I can help you search for cities, explore hotels, find the best deals, or book a room. Ask me anything!",
        ...
        >
          ...
          >
            <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
              ...
              <div>
                <h3 style={{ margin: 0, fontSize: '1.1rem', fontWeight: 'bold' }}>OasisStays AI</h3>
                <span style={{ fontSize: '0.75rem', opacity: 0.85 }}>Concierge Concierge</span>
              </div>
            </div>
            <div style={{ display: 'flex', gap: '0.75rem' }}>
              <button 
                onClick={toggleHistory}
                style={{ background: 'none', border: 'none', color: 'white', cursor: 'pointer', opacity: showHistory ? 1 : 0.8 }}
                onMouseOver={(e) => e.currentTarget.style.opacity = '1'}
                onMouseOut={(e) => e.currentTarget.style.opacity = showHistory ? '1' : '0.8'}
                ...
                historyList.map((item, idx) => (
                  ...
                  >
                    <span style={{ fontSize: '0.75rem', fontWeight: 'bold', color: item.role === 'user' ? 'var(--color-1)' : '#64748b', textTransform: 'uppercase', letterSpacing: '0.05em' }}>
                      {item.role === 'user' ? 'You' : 'OasisStays AI'}
                    </span>
          ...
          {/* Input Footer */}
          <div style={{ display: 'flex', flexDirection: 'column', borderTop: '1px solid rgba(0,0,0,0.05)', background: isExpanded ? 'var(--color-1)' : 'white', borderBottomLeftRadius: '24px', borderBottomRightRadius: '24px' }}>
